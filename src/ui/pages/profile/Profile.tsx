...
const Profile: React.FC = () => {
  ...
  const { profile, userPovs, loadingProfile } = useProfile(id || null);
  ...
  if (!profile) {
    return (
      ...
      sx={{
        ...
        <Button startIcon={<ArrowBackRounded />} variant="outlined" onClick={() => navigate(-1)} sx={{ fontWeight: 700 }}>
          Go Back
        </Button>
      </Stack>
    );
  ...
  const povItems = userPovs?.content || [];
    ...
    >
      ...
      <Grid container spacing={2.5} sx={{ position: "relative", zIndex: 1 }}>
        ...
        <Grid size={{ xs: 12, md: 8, lg: 9 }}>
          ...
          <PovList
            povs={userPovs || { content: [], empty: true }}
            loading={loadingProfile}
            onEdit={() => {}} // Profile page is read-only for others
