...
export const myPovSlice = createSlice({
  ...
  reducers: {
    setMyPovs: (state, action: PayloadAction<{ content: PoV[]; last: boolean; totalElements?: number; size?: number }>) => {
    ...
    addMyPov: (state, action: PayloadAction<PoV>) => {
    ...
    editMyPov: (state, action: PayloadAction<PoV>) => {
    ...
    removeMyPov: (state, action: PayloadAction<string>) => {
